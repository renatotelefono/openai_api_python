...
def create_visual_prompt_from_story(name, country, gender, story_text):
    subject = "man" if gender == "uomo" else "woman"

    base = f"This medieval Norse oil painting from the {country} region portrays {name}, an enigmatic {subject}, "

    visual_elements = []
    if "bosco" in story_text or "foresta" in story_text:
        visual_elements.append("standing in a misty forest")
    if "castello" in story_text:
        visual_elements.append("within a shadowy castle corridor")
    if "mare" in story_text:
        visual_elements.append("overlooking a stormy coastline")
    if "fuoco" in story_text or "ponte" in story_text:
        visual_elements.append("with a flaming bridge in the distance")

    objects = []
    if "runa" in story_text:
        objects.append("his cloak adorned with ancient runes")
    if "libro" in story_text or "cabala" in story_text:
        objects.append("holding a worn Kabbalistic manuscript")

    mood = "a mystical and timeless atmosphere, full of spiritual tension and Norse symbolism"

    full_prompt = (
        base +
        ", ".join(visual_elements) + ". " if visual_elements else ""
        + "He appears with " + ", ".join(objects) + ". " if objects else ""
        + f"The painting evokes {mood}. Style: early medieval oil on panel."
    )
...
def generate_character_image(name, country, birth_year, gender, filename_jpg, story_text):
    ...
    full_prompt = create_visual_prompt_from_story(name, country, gender, story_text)
