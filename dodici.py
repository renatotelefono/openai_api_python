...
def generate_character_image(name, country, birth_year, gender, filename_jpg, story_text):
    # Determina il soggetto in base al genere
    subject = "man" if gender == "uomo" else "woman"

    # Stile in base all'anno di nascita
    if birth_year < 1800:
        style_prompt = (
            f"An oil painting of a {subject} named {name}, from {country}, "
            "with an enigmatic and mystical expression, painted in the style of the period. "
            "The painting should reflect the artistic style of the era."
        )
    elif 1800 <= birth_year <= 1930:
        style_prompt = (
            f"A realistic black and white photograph of a {subject} named {name}, from {country}, "
            "taken during the early 20th century. "
            "The subject stands in a gloomy and decaying environment, such as an old cemetery or ruined castle. "
            "The atmosphere is mysterious, spiritual, and historically rich, with esoteric elements."
        )
    else:
        style_prompt = (
            f"A realistic color photograph of a {subject} named {name}, from {country}, "
            "with a dark and mystical presence. "
            "The setting is eerie and dramatic, such as a cemetery or abandoned castle ruins. "
            "the style is modern photographic realism."
        )

    # Pulizia del testo narrativo (ma ora usato completamente)
    story_excerpt = story_text.strip().replace("\n", " ")
    full_prompt = style_prompt + f" Context: {story_excerpt}"
