...
def generate_character_image(name, country, birth_year, filename_jpg):
    image_prompt = (
        f"Portrait of {name}, a mysterious historical figure born in {country} in {birth_year}, "
        "surrounded by occult symbols, old manuscripts, and mystical atmosphere. "
        "Dark background, Renaissance or Gothic style, oil painting."
    )
...
if __name__ == "__main__":
    ...
    image_path = f"./{image_filename}"  # puoi cambiarlo se vuoi un path tipo ../../assets/

    story = generate_occult_story(name, country, birth_year)
    generate_character_image(name, country, birth_year, image_filename)
